...
class QueueJob {
    ...
    def execute() {
        ...
        if (queuedRuns){
            ...
            // check what steps to include
            def doExtract = (queue.extract == 1) ? true : false
            def doAlign = (queue.align == 1) ? true : false
            def doCombine = (queue.combine == 1) ? true : false
            ...
            try {
                ...
                if (doExtract){
                    ...
                    def configFileExtract = runService.configFileFromRunFolder(run, 'extract')
                    def configFileExtractText = configFileExtract.text.encodeAsBase64().toString()

                    GParsPool.withPool(10) {
                        inputFiles.eachParallel { file ->

                            //stop when config changed
                            if (configFileExtractText == runService.configFileFromRunFolder(run, 'extract').text.encodeAsBase64().toString()){
                                ...
                                def commandExtract = ""
                                    ...
                                    commandExtract += "\"${configFileExtract.canonicalPath}\" " // add config file
                ...
                if (doAlign){
                    ...
                    def configFileAlign = runService.configFileFromRunFolder(run, 'align')

                } else {
                    println "Skipping align"
                }
                ...
                if (doCombine){
                    ...
                    def configFileExtract = runService.configFileFromRunFolder(run, 'combine')

                    // list all matlab files to include in the combine config file
                    def matFiles = []
                    run.eachFile { file ->
