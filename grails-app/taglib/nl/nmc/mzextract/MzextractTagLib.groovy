...
class MzextractTagLib {
    ...
    def settingsForm = { attrs, body ->
        ...
        out << g.form(name:"runProject", action:"run", params:[project:projectSha1, run:runSha1]) {
            ...
            def categories = settings.collect { it.value.category }.unique()
            out << '        <ul id="myTab" class="nav nav-tabs">'
            categories.each { category ->
                out << '        <li><a href="#' + category + '" data-toggle="tab">' + category + '</a></li>'
            }
            out << '        </ul>'

            out << '        <div id="myTabContent" class="tab-content">'

            categories.eachWithIndex { category, idx ->
                out << '            <div class="tab-pane fade in'
                idx != 0 ?: out << ' active'
                out << '" id="' + category + '">'
                out << '                <table>'
                    settings.findAll { it.value.category == category }.each { name, setting ->
                        out << '    <tr>'
                        ...
                        switch(setting.type){
                            case 'select':
                                            out << g.select(name:name, optionKey:"value", optionValue:"label", from:setting.options, value:setting.value)
                                            break;
                            default      :
                                            out << g.field(type:"text", name:name, value:setting.value)
                                            break;
                        }
                        out << '        </td>'
                        out << '    </tr>'
